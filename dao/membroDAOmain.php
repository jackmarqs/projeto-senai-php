<?php
    ...
    class membroDAO{
        ...
        function alterarUsuario(membroDTO $membroDTO){
            $id_usuario = $membroDTO -> getIdUsuario();
            $usuario = $membroDTO -> getUsuario();
            $senha = $membroDTO -> getSenha();
            $perfil = $membroDTO -> getPerfil();
            ...
            $bd = new conexao();
            $conexao = $bd -> getConexao();
            $sql = $conexao -> query("update usuario set user = '$usuario', senha = '$senha', perfil = '$perfil' where id_usuario = '$id_usuario'");
            if(!$sql){
                echo $conexao -> error;
            }else{
                return $sql;
        ...
        function excluirUsuario($id_usuario){
            $bd = new conexao();
            $conexao = $bd -> getConexao();
            $sql = $conexao -> query("delete from usuario where id_usuario = '$id_usuario' ");
            if(!$sql){
                echo $conexao -> error;
            }else{
                return $sql;
